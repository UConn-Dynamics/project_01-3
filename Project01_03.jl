...
begin
    ...
    prob1 = ODEProblem(pendulum_system!, u0, tspan, p1)
	prob2 = ODEProblem(pendulum_system!, u0, tspan, p2)
...
begin
	plot(sol1, idxs=(0, 1), 
	     title="Pendulum Angle Over Time (Ω = $Ω_val and $Ω_val2 rad/s)",
...
begin
    ...
    thetas1 = [u[1] for u in sol1.u]
	thetas2 = [u[1] for u in sol2.u]
    
    # Parametric equations for the mass position
    x_pos1 = (w1 .+ L .* sin.(thetas1)) .* cos.(Ω_val .* ts1)
    y_pos1 = (w1 .+ L .* sin.(thetas1)) .* sin.(Ω_val .* ts1)
    z_pos1 = h1 .- L .* cos.(thetas1)

	# Parametric equations for the mass position
    x_pos2 = (w1 .+ L .* sin.(thetas2)) .* cos.(Ω_val2 .* ts2)
    y_pos2 = (w1 .+ L .* sin.(thetas2)) .* sin.(Ω_val2 .* ts2)
    z_pos2 = h1 .- L .* cos.(thetas2)
    
    # Pivot point coordinates (it rotates too!)
    x_pivot1 = w1 .* cos.(Ω_val .* ts1)
    y_pivot1 = w1 .* sin.(Ω_val .* ts1)
    z_pivot1 = fill(h1, length(ts1))

	x_pivot2 = w1 .* cos.(Ω_val2 .* ts2)
    y_pivot2 = w1 .* sin.(Ω_val2 .* ts2)
    z_pivot2 = fill(h1, length(ts2))
end
...
begin
    ...
    # 3D Plot
    plt1 = plot(x_pos1, y_pos1, z_pos1, 
	...
	plt2 = plot!(x_pos2, y_pos2, z_pos2, 
        line=(1, :pink, :dash), label="Path of Mass",
        xlabel="X (m)", ylabel="Y (m)", zlabel="Z (m)",
        title="3D Pendulum Motion (Ω = $Ω_val2)",
        xlim=(-0.3, 0.3), ylim=(-0.3, 0.3), zlim=(0, 0.4),
        camera=(45, 30))
    
    # Draw the rotating arm/frame (Vertical pole to pivot)
    plot!([0, 0], [0, 0], [0, h1], color=:black, lw=3, label="Main Axis")
    plot!([0, x_pivot1[t_set]], [0, y_pivot1[t_set]], [h1, h1], color=:green, lw=3, label="Rotating Arm (w1)")

	plot!([0, 0], [0, 0], [0, h1], color=:black, lw=3, label="Main Axis")
    plot!([0, x_pivot2[t_set]], [0, y_pivot2[t_set]], [h1, h1], color=:green, lw=3, label="Rotating Arm (w1)")
    
    # Draw the pendulum (Pivot to Mass)
    plot!([x_pivot1[t_set], x_pos1[t_set]], 
          [y_pivot1[t_set], y_pos1[t_set]], 
          [z_pivot1[t_set], z_pos1[t_set]], 
          color=:red, lw=4, label="Pendulum (L)")

	plot!([x_pivot2[t_set], x_pos2[t_set]], 
          [y_pivot2[t_set], y_pos2[t_set]], 
          [z_pivot2[t_set], z_pos2[t_set]], 
          color=:red, lw=4, label="Pendulum (L)")
    
    # Add the mass point
    scatter!([x_pos1[t_set]], [y_pos1[t_set]], [z_pos1[t_set]], 
             color=:red, markersize=6, label="Mass (m)")

	scatter!([x_pos2[t_set]], [y_pos2[t_set]], [z_pos2[t_set]], 
             color=:red, markersize=6, label="Mass (m)")
...
begin
    anim = @animate for i in 1:3:length(ts2)

        plt1 = plot(
            x_pos1[1:i], y_pos1[1:i], z_pos1[1:i],
            line=(2, :blue),
            xlabel="X (m)", ylabel="Y (m)", zlabel="Z (m)",
            title="Spinning Pendulum (Ω = $(Ω_val) and $(Ω_val2) rad/s)",
            xlim=(-0.3, 0.3),
            ylim=(-0.3, 0.3),
            zlim=(0, 0.4),
            camera=(45, 30),
		...
		plt2 = plot!(
            x_pos2[1:i], y_pos2[1:i], z_pos2[1:i],
            line=(2, :pink),
            xlabel="X (m)", ylabel="Y (m)", zlabel="Z (m)",
            xlim=(-0.3, 0.3),
            ylim=(-0.3, 0.3),
        ...
        # Pendulum rod
        plot!([x_pivot1[i], x_pos1[i]],
              [y_pivot1[i], y_pos1[i]],
              [z_pivot1[i], z_pos1[i]],
              color=:red, lw=4)

		plot!([x_pivot2[i], x_pos2[i]],
              [y_pivot2[i], y_pos2[i]],
              [z_pivot2[i], z_pos2[i]],
              color=:red, lw=4)
