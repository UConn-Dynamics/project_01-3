...
begin
	...
	using OrdinaryDiffEq          # Time integration of ODEs
	using Plots                   # Plotting and animations
	
	gravity = 9.81                # [m/s^2]
	pendulum_length = 0.15        # L, [m]
	frame_height = 0.20           # h1, [m]
	frame_width = 0.10            # w1, [m]
	mass_point = 0.10             # m, [kg] (drops out of the EOM)
...
# ╔═╡ c85f9fdf-ac29-4e70-84f6-3d9bc5de4f0e


function spinning_pendulum_eom!(state_derivative, state, parameters, time)
    angle = state[1]                    # θ
    angular_velocity = state[2]         # θ̇
    gravity, pendulum_length, frame_width, rotation_speed = parameters

    angle_acceleration = (rotation_speed^2 / 2) * sin(2 * angle) +
                         (rotation_speed^2 * frame_width / pendulum_length) * cos(angle) -
                         (gravity / pendulum_length) * sin(angle)

    state_derivative[1] = angular_velocity
    state_derivative[2] = angle_acceleration
end



# ╔═╡ 87c72804-cf37-4717-9733-98205d5a75b0
begin
	
	time_span = (0.0, 20.0)                     # [s] total simulation time
	initial_state = [0.2, 0.0]                  # [θ(0), θ̇(0)]
	
	slow_rotation_speed = 1.0                   # Ω_slow [rad/s]
	fast_rotation_speed = 3.0                  # Ω_fast [rad/s]
	
	slow_parameters = (gravity, pendulum_length, frame_width, slow_rotation_speed)
	fast_parameters = (gravity, pendulum_length, frame_width, fast_rotation_speed)
	
	slow_problem = ODEProblem(spinning_pendulum_eom!, initial_state, time_span, slow_parameters)
	fast_problem = ODEProblem(spinning_pendulum_eom!, initial_state, time_span, fast_parameters)
	
	slow_solution = solve(slow_problem, Tsit5(); reltol = 1e-8, abstol = 1e-8)
	fast_solution = solve(fast_problem, Tsit5(); reltol = 1e-8, abstol = 1e-8)
	
end

# ╔═╡ 82e84d45-42e3-4e4f-95eb-de4c5e43be07
begin
	
	time_points = range(time_span[1], time_span[2], length = 1000)
	
	slow_angles = [slow_solution(t)[1] for t in time_points]
	fast_angles = [fast_solution(t)[1] for t in time_points]
	
	plot(time_points, slow_angles;
	     label = "θ(t), Ω = $(slow_rotation_speed) rad/s",
	     xlabel = "time [s]", ylabel = "angle θ [rad]",
	     legend = :topright)
	
	plot!(time_points, fast_angles;
	      label = "θ(t), Ω = $(fast_rotation_speed) rad/s")
	
end

# ╔═╡ 0284a7d4-d0a0-4eb5-98fc-447cf6afe437

function mass_position_global(time, angle, rotation_speed)
    phi = rotation_speed * time        # frame rotation angle
    base_x = frame_width * cos(phi)
    base_y = frame_width * sin(phi)
    base_z = frame_height

    mass_x = base_x + pendulum_length * sin(angle) * cos(phi)
    mass_y = base_y + pendulum_length * sin(angle) * sin(phi)
    mass_z = base_z - pendulum_length * cos(angle)

    return mass_x, mass_y, mass_z, base_x, base_y, base_z
end


# ╔═╡ e31f7bb9-f112-4638-af7f-4e2815cf2321
begin
	
	function animate_spinning_pendulum(solution, rotation_speed, file_name)
	    time_points = range(0.0, 10.0, length = 200)
	
	    animation = @animate for time in time_points
	        angle = solution(time)[1]
	        mass_x, mass_y, mass_z, base_x, base_y, base_z =
	            mass_position_global(time, angle, rotation_speed)
	
	        plot3d([0.0, base_x, mass_x],
	               [0.0, base_y, mass_y],
	               [0.0, base_z, mass_z];
	               xlims = (-0.3, 0.3), ylims = (-0.3, 0.3), zlims = (0.0, 0.4),
	               xlabel = "x [m]", ylabel = "y [m]", zlabel = "z [m]",
	               marker = [:none, :none, :circle],
	               title = "Spinning pendulum, Ω = $(rotation_speed) rad/s",
	               legend = false)
	    end
	
	    gif(animation, file_name, fps = 30)
	end
	
	animate_spinning_pendulum(slow_solution, slow_rotation_speed, "slow_pendulum.gif")
	animate_spinning_pendulum(fast_solution, fast_rotation_speed, "fast_pendulum.gif")
	``
	
end

# ╔═╡ da3f7b46-9921-4c36-af1f-22477f023775
begin
	time_values = range(0.0, 10.0, length = 200)  # times for animation frames
	
	@gif for current_time in time_values
	    angle_value = slow_solution(current_time)[1]
	    mass_x, mass_y, mass_z, base_x, base_y, base_z =
	        mass_position_global(current_time, angle_value, slow_rotation_speed)
	    ...
	    plot3d([0.0, base_x, mass_x],
	           ...
	           marker = [:none, :none, :circle],
	           xlabel = "x [m]", ylabel = "y [m]", zlabel = "z [m]",
	           title = "Spinning pendulum – slow rotation (Ω = $(slow_rotation_speed) rad/s)",
	           legend = false)
	end
end

# ╔═╡ cf6920aa-dd86-41f7-8bfc-e4edc851f334
begin
time_values2 = range(0.0, 10.0, length = 200)

@gif for current_time in time_values2
    angle_value = fast_solution(current_time)[1]
    mass_x, mass_y, mass_z, base_x, base_y, base_z =
        mass_position_global(current_time, angle_value, fast_rotation_speed)
    ...
    plot3d([0.0, base_x, mass_x],
           ...
           marker = [:none, :none, :circle],
           xlabel = "x [m]", ylabel = "y [m]", zlabel = "z [m]",
           title = "Spinning pendulum – fast rotation (Ω = $(fast_rotation_speed) rad/s)",
           legend = false)
end
end
...
manifest_format = "2.0"
project_hash = "d89a15438cf6c63fb8f0fa4422b7b7e4bd30dc7f"
