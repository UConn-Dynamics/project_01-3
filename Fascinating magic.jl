...
# ╔═╡ ffe58d15-daff-4f22-9ef9-b363e074d604
### A Pluto.jl notebook ###
# v0.20.21

using Markdown

# ╔═╡ 7dfebcd3-4bff-4546-87ad-23442a5cc938
using InteractiveUtils

# ╔═╡ d42471d9-0dce-4764-a0d0-65a07b146d46
using DifferentialEquations, Plots, PlutoUI

# ╔═╡ cc9dce14-6477-4a9f-8dd8-59dfd45928ae
#ADDED
using Symbolics, Latexify, OrdinaryDiffEq

# ╔═╡ 8db78cd8-129d-4cb2-aa37-febcc8ec93c6
...
# ╔═╡ b858369f-647e-49d8-9fda-324dcb5163c4
begin
...
# ╔═╡ 4fa34578-ecdb-426f-bcbe-906544f85cab
# ADDED BY JULIA
...
# ╔═╡ b0e9e36c-8481-4d79-bc13-1b13ef4bf68c
...
# ╔═╡ 9322b3b5-088a-42bb-94e0-c5f93eea37df


# ╔═╡ 67a0f50c-c535-4f89-ae43-450f5c53601a
begin
...
# ╔═╡ bb10f2cf-935e-4551-a3e2-6db613feb281
plot(sol, vars=(0, 1), 
     title="Pendulum Angle Over Time (Ω = $Ω_val rad/s)",
     xlabel="Time (s)", ylabel="Theta (rad)",
     lw=2, legend=false)

# ╔═╡ 87d458bf-777c-4898-aed7-6cc7a5900d4c
begin
    ts = sol.t
    thetas = [u[1] for u in sol.u]
    
    # Parametric equations for the mass position
    x_pos = (w1 .+ L .* sin.(thetas)) .* cos.(Ω_val .* ts)
    y_pos = (w1 .+ L .* sin.(thetas)) .* sin.(Ω_val .* ts)
    z_pos = h1 .- L .* cos.(thetas)
    
    # Pivot point coordinates (it rotates too!)
    x_pivot = w1 .* cos.(Ω_val .* ts)
    y_pivot = w1 .* sin.(Ω_val .* ts)
    z_pivot = fill(h1, length(ts))
end

# ╔═╡ a196a6b9-ce10-43c5-b01c-3810edd17703
begin
    time_idx = 0.5
	t_set = 5
    ...
    plt = plot(x_pos, y_pos, z_pos, 
        line=(1, :blue, :dash), label="Path of Mass",
        xlabel="X (m)", ylabel="Y (m)", zlabel="Z (m)",
        title="3D Pendulum Motion (Ω = $Ω_val)",
        xlim=(-0.3, 0.3), ylim=(-0.3, 0.3), zlim=(0, 0.4),
        camera=(45, 30))
    
    # Draw the rotating arm/frame (Vertical pole to pivot)
    plot!([0, 0], [0, 0], [0, h1], color=:black, lw=3, label="Main Axis")
    plot!([0, x_pivot[t_set]], [0, y_pivot[t_set]], [h1, h1], color=:green, lw=3, label="Rotating Arm (w1)")
    
    # Draw the pendulum (Pivot to Mass)
    plot!([x_pivot[t_set], x_pos[t_set]], 
          [y_pivot[t_set], y_pos[t_set]], 
          [z_pivot[t_set], z_pos[t_set]], 
          color=:red, lw=4, label="Pendulum (L)")
    
    # Add the mass point
    scatter!([x_pos[t_set]], [y_pos[t_set]], [z_pos[t_set]], 
             color=:red, markersize=6, label="Mass (m)")
    
    plt
end

# ╔═╡ be3c31fc-15ec-48bf-925b-5a4b92475a64
begin
    anim = @animate for i in 1:3:length(ts)

        plt = plot(
            x_pos[1:i], y_pos[1:i], z_pos[1:i],
            line=(2, :blue),
            xlabel="X (m)", ylabel="Y (m)", zlabel="Z (m)",
            title="Spinning Pendulum (Ω = $(Ω_val) rad/s)",
            xlim=(-0.3, 0.3),
            ylim=(-0.3, 0.3),
            zlim=(0, 0.4),
            camera=(45, 30),
...
# ╔═╡ 2c140f29-da18-46f2-be2f-38887be23a95

#ADDED
# Optional: Solve also for Ω_val2 and overlay θ(t)
begin
...
# ╔═╡ 82dd18bd-3a6d-4c48-be21-77fe40b514f1


# ╔═╡ 1e5cfa5f-df96-4d16-aeb3-9490f1234f1d
...
manifest_format = "2.0"
project_hash = "305b49324b8fb420e2c4e9115abf50572d4175e2"
...
git-tree-sha1 = "43e7dee8f79548f9552744c4a324dabdd382ba0c"
